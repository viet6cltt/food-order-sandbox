...
const SignupForm: React.FC<{ className?: string }> = ({ className = '' }) => {
  ...
  // Reset Firebase auth on unmount
  useEffect(() => {
    return () => {
      firebaseAuth.reset();
    };
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Mã OTP không đúng. Vui lòng thử lại.';
      if (
        errorMessage.includes('Chưa gửi mã OTP') ||
        errorMessage.includes('gửi mã OTP trước')
      ) {
        setStep('phone');
        setOtpCode('');
        setError('Vui lòng gửi mã OTP trước khi xác minh.');
